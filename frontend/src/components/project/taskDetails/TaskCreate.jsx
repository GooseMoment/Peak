...
const TaskCreate = () => {
  ...
  const makeTask = async () => {
    try {
      ...
      queryClient.invalidateQueries({
        queryKey: ["drawers", { projectID: state?.project_id }],
      });
      queryClient.invalidateQueries({
        queryKey: ["projects", state?.project_id],
      });
      onClose();
    } catch (e) {
      if (newTask?.name) toast.error("할 일 생성에 실패하였습니다.");
      else toast.error("할 일의 이름을 적어주세요.");
    }
  };
